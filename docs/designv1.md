...
- 标准模式与 Widget 模式分离渲染，可双窗口并行使用。
- 标准模式支持打开 Widget、透明度调节、任务筛选与排序展示。
- Widget 支持透明背景、无边框、置顶、锁定/解锁、解锁拖动。
- Widget 锁定时启用点击穿透，解锁时可交互并可拖动窗口。
- 已接入全局快捷键 `Ctrl+Shift+L`，用于切换 Widget 锁定状态。
- 主窗口点击关闭按钮时，行为为最小化（非退出）。
- 已接入托盘菜单：打开标准窗口 / 锁定或解锁 Widget / 退出应用。
- 托盘“锁定/解锁 Widget”已改为动态文案，随当前状态变化。
- 锁定状态与 Widget 位置已做持久化，应用重启后可恢复。

### 3.2 变更确认

- 已移除“每日 6 点自动延期”相关设计与实现要求。
- 不再维护 `lastAutoCheckTime`。
- 不再进行“跨天补算”自动延期逻辑。
